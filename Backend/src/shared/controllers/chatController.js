...
const sendMessage = async (req, res) => {
  try {
    ...
    const receiver = await User.findById(receiverId).select('role email profile.firstName profile.lastName');
    const receiverName = receiver ? `${receiver.profile.firstName} ${receiver.profile.lastName}`.trim() : 'User';
