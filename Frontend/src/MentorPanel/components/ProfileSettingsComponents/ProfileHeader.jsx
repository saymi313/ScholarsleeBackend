...
const ProfileHeader = () => {
    ...
    try {
      ...
      if (userResponse.data.success) {
        ...
        setProfileData(prev => ({
          ...
          location: userData.profile.country || 'Lahore, Pakistan'
        }))
      }
      ...
      if (mentorResponse.data.success) {
        const mentorData = mentorResponse.data.data.profile
        setProfileData(prev => ({
          ...prev,
          title: mentorData.title || 'Software Engineer'
        }))
      }
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const mentorData = {
        ...
        bio: profileData.title.length >= 50 ? profileData.title : `${profileData.title}. Experienced professional with expertise in software development and mentoring. Passionate about helping others grow in their careers.`
      }

      console.log('🎓 Updating mentor profile:', mentorData)
      const mentorResponse = await profileAPI.mentor.update(mentorData)
      console.log('🎓 Mentor update response:', mentorResponse.data)

      if (!mentorResponse.data.success) {
        // If mentor profile doesn't exist, create it
        if (mentorResponse.data.message && mentorResponse.data.message.includes('not found')) {
          console.log('🆕 Creating new mentor profile...')
          const createResponse = await profileAPI.mentor.create({
            title: profileData.title,
            bio: profileData.title.length >= 50 ? profileData.title : `${profileData.title}. Experienced professional with expertise in software development and mentoring. Passionate about helping others grow in their careers.`,
            education: [],
            experience: [],
          ...
          if (!createResponse.data.success) {
            throw new Error(createResponse.data.message || 'Failed to create mentor profile')
          }
        } else {
          throw new Error(mentorResponse.data.message || 'Failed to update mentor profile')
        }
      }
  ...
  const handleCancel = () => {
    ...
    setProfileImage("https://i.pravatar.cc/300?img=12")
    setHasChanges(false)
    setIsEditing(false)
  }
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-4 sm:gap-6">
          ...
          <div className="w-full lg:w-auto bg-[#242424] rounded-xl p-3 sm:p-4 border border-[#3a3a3a] lg:min-w-[120px]">
            <div className="text-center">
              <div className="text-2xl sm:text-3xl font-bold text-[#5D38DE] mb-1">85%</div>
            ...
            <div className="mt-2 sm:mt-3 w-full bg-[#1a1a1a] rounded-full h-1.5 sm:h-2">
              <div
                className="bg-gradient-to-r from-[#5D38DE] to-[#8b5cf6] h-1.5 sm:h-2 rounded-full"
                style={{ width: "85%" }}
              ></div>
            </div>
          </div>
