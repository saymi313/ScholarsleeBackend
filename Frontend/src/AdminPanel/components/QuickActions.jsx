...
export default function QuickActions() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data?.success) {
          ...
          setError(response.data?.message || "We couldn't load transactions. Please try again.")
        }
      } catch (err) {
        console.error("Error fetching transactions chart:", err)
        setError(err.message || "We couldn't load transactions. Please try again.")
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-[#161619] p-3 h-64 lg:h-72 flex flex-col">
