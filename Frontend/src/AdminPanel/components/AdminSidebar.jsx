...
export default function AdminSidebar({ open, onClose }) {
  return (
    <aside
      className={`z-40 bg-[#121214] border-r border-white/10 w-72 shrink-0 h-screen fixed md:sticky md:top-0 transform transition-transform duration-300 ${
    ...
    >
      ...
      <nav className="py-4 h-[calc(100vh-4rem)] overflow-y-auto">
        <ul className="space-y-2 px-3">
          {[...links].sort((a, b) => a.label.localeCompare(b.label)).map((l) => (
            <li key={l.href}>
              <NavLink
                to={l.href}
                className={({ isActive }) =>
                  `flex items-center gap-3 rounded-md px-3 py-2 text-sm ${
                    isActive ? "bg-[#5D38DE]/20 text-white" : "text-white/80 hover:text-white hover:bg-white/5"
                  }`
                }
              >
